...
class PromptTrainDataset(Dataset):
    ...
    def _init_snow_ids(self):
        snow_list = self.args.data_file_dir + "snowy/snowTrain.txt"
        lines = [l.strip() for l in open(snow_list)]
        self.snow_ids = [
            {"clean_id": self.args.desnow_dir + fname, "de_type": 6}
            for fname in lines
        ]
        # you can oversample if you like:
        # self.snow_ids *= 120
        self.num_snow = len(self.snow_ids)
        print(f"Total Snowy Ids: {self.num_snow}")
    ...
    def _get_sngt_name(self, rainy_name):
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                # Rain Streak Removal
                degrad_img = crop_img(np.array(Image.open(sample["clean_id"]).convert('RGB')), base=16)
                clean_name = self._get_sngt_name(sample["clean_id"])
                clean_img = crop_img(np.array(Image.open(clean_name).convert('RGB')), base=16)
