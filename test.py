...
if __name__ == '__main__':
    ...
    parser.add_argument('--all_path', type=str, default="data/test/degraded/")
    ...
    np.random.seed(0)
    torch.manual_seed(0)
    print(torch.__version__)       # likely ends in '+cpu'
    ...
    if testopt.mode == 0:
        ...
        testopt.test_path = testopt.all_path
        print(testopt.test_path )
        all_set = TestSpecificDataset(testopt)
        loader  = DataLoader(all_set, batch_size=1,
                            shuffle=False, num_workers=0)

        # 2) make an output folder
        out_dir = os.path.join(testopt.output_path, 'all/')
        os.makedirs(out_dir, exist_ok=True)

        # 3) run inference & save
        print(f"Running inference on folder {testopt.all_path}, saving to {out_dir}")
        with torch.no_grad():
            for ([name], degrad) in tqdm(loader):
                degrad = degrad.cuda()
                restored = net(degrad)
                save_image_tensor(restored, os.path.join(out_dir, name[0] + '.png'))
